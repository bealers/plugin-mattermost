...
# Mattermost Development Environment Initialization Script
# Automatically creates admin user, team, bot account, and development channels
...
TEAM_NAME="dev-team"
TEAM_DISPLAY_NAME="Development Team"
BOT_USERNAME="elizaos-bot"
BOT_DISPLAY_NAME="ElizaOS Development Bot"

echo "Initializing Mattermost development environment..."
...
while ! curl -s "$MATTERMOST_URL/api/v4/system/ping" >/dev/null 2>&1; do
    if [ $timeout -le 0 ]; then
        ...
        echo "Make sure containers are running: docker-compose -f docker-compose.dev.yml up -d"
        exit 1
    fi
    sleep 2
    timeout=$((timeout-2))
...
BOT_RESPONSE=$(curl -s -X POST "$MATTERMOST_URL/api/v4/bots" \
    ...
    -d "{
        ...
        \"description\": \"ElizaOS development bot for local testing\"
    }")
...
TOKEN_RESPONSE=$(curl -s -X POST "$MATTERMOST_URL/api/v4/users/$BOT_USER_ID/tokens" \
    ...
    -H "Content-Type: application/json" \
    -d "{
        \"description\": \"ElizaOS development token\"
    }")
...
if echo "$TEAM_MEMBER_RESPONSE" | grep -q "team_id"; then
    ...
    if echo "$BOT_TEAMS" | grep -q "$TEAM_ID"; then
        ...
        echo "This will cause development testing to fail!"
        exit 1
    fi
fi

# Create development channels
echo "Creating development channels..."
for channel in "general" "elizaos-dev" "plugin-testing"; do
...
# Generate development configuration snippet
echo "Generating development configuration snippet..."
cat > .env.dev.mattermost.snippet << EOF
# Mattermost Development Configuration
# Generated on $(date)
# Add these variables to your existing .env setup

# Mattermost Server Configuration (containerised development)
MATTERMOST_URL=http://localhost:8065
...
# Admin credentials (for manual testing via web interface)
# Access http://localhost:8065 with these credentials:
# Username: $ADMIN_USERNAME
# Password: $ADMIN_PASSWORD
EOF

echo ""
echo "Mattermost development environment initialized successfully!"
echo ""
echo "Summary:"
...
echo ""
echo "Configuration snippet generated: .env.dev.mattermost.snippet"
echo "Add these variables to your existing environment setup"
echo ""
echo "Access Mattermost web interface: http://localhost:8065"
echo "Ready to test with: elizaos dev --character ../hiro.character.json"
echo "" 